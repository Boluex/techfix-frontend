...
export const DownloadSection = () => {
  ...
  // Backend proxy URLs (hides GitHub URLs)
  const LINUX_URL = "https://techfixai-backend.onrender.com/download/agent/linux";
  const WINDOWS_URL = "https://techfixai-backend.onrender.com/download/agent/windows";
  ...
  const trackDownload = async (platform: string) => {
    try {
      await fetch('https://techfixai-backend.onrender.com/track-download', {
        ...
        body: JSON.stringify({ type: platform, version: '1.0.0' }),
      });
  ...
  const handleDownload = async (platform: string, url: string) => {
    ...
    // Trigger download via backend proxy
          ...
          <div>
            ...
            <div className="mt-8 p-6 bg-muted/30 rounded-xl border border-border/50">
              ...
              <ol className="text-sm text-muted-foreground space-y-2 list-decimal list-inside">
                ...
                <li>
                  <strong>Windows:</strong> Double-click <code className="bg-muted px-1 rounded">TechFixAgent.exe</code>
                </li>
                <li>
                  <strong>Linux:</strong> Run <code className="bg-muted px-1 rounded">chmod +x run.sh && ./run.sh</code>
                </li>
                <li>Enter your 8-digit service token when prompted</li>
              </ol>
            </div>

            <div className="mt-6 p-4 bg-yellow-50/50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                💡 Send us feedback: <a href="https://tally.so/r/mR7ydj" className="font-bold underline hover:text-yellow-900 dark:hover:text-yellow-100">Click Here</a>
              </p>
            </div>
          </div>
